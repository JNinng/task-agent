...
在配置文件中启用并填写 Nacos 连接信息，然后在代码中将 Nacos Source 传入 `config.Init`：
...
func main() {
    // 创建 Nacos 配置源
    nacosSource := nacos.NewSource(&config.NacosConfig{
        Addr:      "127.0.0.1",
        Port:      8848,
        Username:  "nacos",
        Password:  "nacos",
        Namespace: "public",
        Group:     "DEFAULT_GROUP",
        DataId:    "template-app.yml",
        LogLevel:  "debug",
        LogDir:    "./logs",
        CacheDir:  "./cache",
    })

    // 传入 config.Init，Nacos 配置将合并到本地配置之上
    // Nacos 中的配置项优先级更高
    cfg, err := config.Init("configs/config.yaml", nacosSource)
    if err != nil {
        panic(err)
    }
